...
public class KclusterEdge extends BidirectionalEdge {
	@Override
	public void draw(Graphics g, PositionTransformation pt) {
		...
		case MIST:
			if (startNode instanceof KclusterNode && endNode instanceof KclusterNode) {
				...
				if (deb.getMISTData().parent == fin.ID || fin.getMISTData().parent == deb.ID) {
					resolved = true;
					Graphics2D g2 = (Graphics2D) g;
					Stroke stroke = new BasicStroke(5f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f);
					g2.setStroke(stroke);
		...
		case CLR:
			if (startNode instanceof KclusterNode && endNode instanceof KclusterNode) {
				...
				if (deb.getCLRData().parent == fin.ID || fin.getCLRData().parent == deb.ID) {
					resolved = true;
					Graphics2D g2 = (Graphics2D) g;
					Stroke stroke = new BasicStroke(5f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f);
					g2.setStroke(stroke);
